...
import { AdminType, PaymentStatus, Role, } from "../../../../generated/prisma/enums.js";
...
const generateFacultyId = (department) => `${department}-${Date.now().toString(36).toUpperCase()}`;
const hashPassword = (password) => bcrypt.hash(password, Number(config.bcrypt_salt_rounds) || 10);
const createFaculty = async (payload, actor) => {
...
const createAdmin = async (payload, actor) => {
    ...
    const isAllowedToCreateAdmins = actingAdmin?.role === Role.ADMIN &&
        (actingAdmin.adminType === AdminType.VC ||
            actingAdmin.adminType === AdminType.SUPER);
    if (!isAllowedToCreateAdmins) {
        throw new AppError(httpStatus.FORBIDDEN, "Only the VC or a super admin can create admin accounts");
    }
    const email = payload.email.trim().toLowerCase();
    const isUserExist = await prisma.user.findUnique({ where: { email } });
    if (isUserExist) {
...
const assignDepartmentHead = async (facultyProfileId, actor) => {
    ...
    if (faculty.isDepartmentHead) {
        throw new AppError(httpStatus.BAD_REQUEST, "This faculty member is already the department head");
    }
...
const updateUserStatus = async (userId, payload, actor) => {
...
const getDashboardStats = async () => {
    const [totalStudents, totalFaculty, totalAdmins, totalPaidFees, totalPendingFees] = await Promise.all([
