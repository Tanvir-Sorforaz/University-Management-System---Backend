...
const otpKeyFor = (email) => `studen-registration-otp:${email}`;
const pendingDataKeyFor = (email) => `student-registration-data${email}`;
const issueTokens = (jwtPayload) => {
...
const registerStudent = async (payload) => {
    ...
    await redisClient.set(pendingDataKeyFor(email), JSON.stringify(pendingPayload), { expiration: { type: "EX", value: OTP_EXPIRATION_SECOND } });
...
const verifyStudentEmail = async (payload) => {
    const email = payload.email.trim().toLowerCase();
    const isUserExist = await prisma.user.findUnique({ where: { email } });
    if (isUserExist) {
    ...
    const pendingPayload = JSON.parse(redisPendingData);
...
const refreshToken = async (token) => {
    ...
    if (!verified.success || !verified.data) {
        throw new AppError(httpStatus.UNAUTHORIZED, config.node_env === "development" ? verified.error : "Invalid refresh token");
    }
